...
export default function AdminPanel({ cfg, questions, onSave, onClose }) {
  ...
  const updateSite = (key, val) => setLocalCfg(c => ({ ...c, site: { ...c.site, [key]: val } }));
  ...
  const addQ = () => setLocalQs(qs => [...qs, {
    q: "New question", opts: ["Option A", "Option B", "Option C", "Option D"],
    correct: 0, explain: "Explanation here."
  }]);
  ...
  const tabs = ["branding", "emailjs", "site", "questions", "settings"];
      ...
      <div className="admin-modal">
        ...
        <div className="box box-warning" style={{ marginBottom: "1rem" }}>
          <p style={{ fontSize: "0.82rem" }}>
            <strong>To save permanently:</strong> update src/data/config.js and redeploy to Vercel.
          </p>
        </div>
        ...
        {tab === "branding" && (
          <div>
            <div className="admin-sec-title">Organisation and branding</div>
            ...
            <div className="grid-2">
              ...
              <div className="admin-field"><label>Secondary colour</label>
            ...
            <div className="admin-field"><label>QHSE Manager name</label>
              <input value={localCfg.qhseManager} onChange={e => updateCfg("qhseManager", e.target.value)} /></div>
          ...
          <div>
            ...
            <div className="grid-2">
              <div className="admin-field"><label>Service ID</label>
                <input value={localCfg.emailjs.serviceId} onChange={e => updateEmailjs("serviceId", e.target.value)} style={{ fontFamily: "monospace" }} /></div>
              <div className="admin-field"><label>Template ID</label>
                <input value={localCfg.emailjs.templateId} onChange={e => updateEmailjs("templateId", e.target.value)} style={{ fontFamily: "monospace" }} /></div>
            </div>
            <div className="grid-2">
              <div className="admin-field"><label>Public Key</label>
                <input value={localCfg.emailjs.publicKey} onChange={e => updateEmailjs("publicKey", e.target.value)} style={{ fontFamily: "monospace" }} /></div>
              <div className="admin-field"><label>Send completion emails to</label>
                <input type="email" value={localCfg.emailjs.toEmail} onChange={e => updateEmailjs("toEmail", e.target.value)} /></div>
            </div>
          </div>
        )}
        ...
        {tab === "site" && (
          <div>
            <div className="admin-sec-title">Site details</div>
            <div className="grid-2">
              <div className="admin-field"><label>Site name</label>
                <input value={localCfg.site.name} onChange={e => updateSite("name", e.target.value)} /></div>
              <div className="admin-field"><label>Legal entity</label>
                <input value={localCfg.site.entity} onChange={e => updateSite("entity", e.target.value)} /></div>
            </div>
            <div className="admin-field"><label>Address</label>
              <input value={localCfg.site.address} onChange={e => updateSite("address", e.target.value)} /></div>
            <div className="grid-2">
              <div className="admin-field"><label>QHSE Manager</label>
                <input value={localCfg.site.qhseManager} onChange={e => updateSite("qhseManager", e.target.value)} /></div>
              <div className="admin-field"><label>Responsible Person</label>
                <input value={localCfg.site.responsiblePerson} onChange={e => updateSite("responsiblePerson", e.target.value)} /></div>
            </div>
          </div>
        )}
